...
const DrumPad = (props) => {
    ...
    return (
        <div id={`parent-id-${props.id}`}>
            <button onClick={()=>{if(isPowerOn) props.handleClick(props.id)}} className={`drum-pad ${(isKeyDown && keyPressed === props.id.toLowerCase() && isPowerOn) ? "good-vibes-bro" : ""}`}>{props.id}</button>
